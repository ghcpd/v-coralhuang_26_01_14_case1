...
class TestFake(unittest.TestCase):
    ...
    def test_os_replacements_preserved(self):
        ua = UserAgent(os=["windows"])
        assert ua.os == settings.OS_REPLACEMENTS["windows"]

    def test_numeric_conversion_to_float(self):
        ua = UserAgent(min_version=1, min_percentage=5)
        assert isinstance(ua.min_version, float)
        assert isinstance(ua.min_percentage, float)
        assert ua.min_version == 1.0
        assert ua.min_percentage == 5.0

    def test_safe_attrs_allows_empty_iterables(self):
        ua = UserAgent(safe_attrs=[])
        assert ua.safe_attrs == set()
...
@pytest.mark.parametrize("iterable_factory", [list, tuple, set])
def test_iterable_flexibility_browsers(iterable_factory):
    ua = UserAgent(browsers=iterable_factory(["chrome", "edge"]))
    assert {"chrome", "edge"}.issubset(_as_set(ua.browsers))


@pytest.mark.parametrize("iterable_factory", [list, tuple, set])
def test_iterable_flexibility_os(iterable_factory):
    ua = UserAgent(os=iterable_factory(["linux", "windows"]))
    # windows should expand; ensure linux preserved
    assert "linux" in ua.os
    assert "win10" in ua.os and "win7" in ua.os


@pytest.mark.parametrize("iterable_factory", [list, tuple, set])
def test_iterable_flexibility_platforms(iterable_factory):
    ua = UserAgent(platforms=iterable_factory(["pc", "mobile"]))
    assert {"pc", "mobile"}.issubset(_as_set(ua.platforms))


def test_invalid_iterables_raise_value_error_for_empty():
    with pytest.raises(ValueError):
        UserAgent(browsers=[])
    with pytest.raises(ValueError):
        UserAgent(os=[])
...
def test_invalid_iterables_raise_type_error_for_wrong_types():
...
def test_no_assertion_error_from_init():
    # Ensure __init__ uses explicit exceptions instead of assertions
    with pytest.raises((TypeError, ValueError)) as excinfo:
        UserAgent(browsers=object())
    assert not isinstance(excinfo.value, AssertionError)
